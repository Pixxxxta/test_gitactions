...
class CourtDebtsPage:
    ...
    def choose_region(self, region):
        self._helpers.wait_and_click('//android.widget.Button[@text="Регион для проверки долгов Республика Адыгея ('
                                     'Адыгея)"]')
        time.sleep(1)
        self._helpers.wait_for_element('//android.widget.EditText').send_keys(region)
        time.sleep(1)
        self._helpers.wait_and_click(
            '//android.widget.ListView/android.view.View[2]/android.view.View/android.view.View/android.view.View')
    ...
    def verify_no_arrears(self, timeout=20):
        ...
        try:
            no_arrears_check = self._helpers.wait_for_element(
                '//android.widget.TextView[@text="ЗAДОЛЖЕННОСТЕЙ НЕТ, ПОЗДРАВЛЯЕМ!"]',
                timeout)
