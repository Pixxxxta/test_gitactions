...
class StateDutyPage:
    ...
    def set_doc_num(self, doc_num):
        self._helpers.wait_for_element('//android.widget.EditText[@resource-id="Номер документа"]').send_keys(doc_num)

    def set_fio(self, fio):
        self._helpers.wait_for_element('//android.widget.EditText[@resource-id="ФИО (на русском языке)"]').send_keys(fio)
