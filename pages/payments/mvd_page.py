...
class MVDPage:
    ...
    def set_data(self, doc_category='Паспорт иностранного гражданина', doc_number='FA6169953'):
        ...
        try:
            ...
            document_number_input = self._helpers.wait_for_element('//android.widget.EditText')
    ...
    def verify_no_fines(self, timeout=20):
        """Проверка, что штрафов нет"""
        try:
            no_fines_check = self._helpers.wait_for_element('//android.widget.TextView[@text="ШТРАФОВ НЕТ, '
                                                            'ПОЗДРАВЛЯЕМ!"]', timeout)
